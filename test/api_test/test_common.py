...
class CommonApiTestCase(AppContextMixin, TestCase):
    ...
    def test_single_upload(self, mock_save: Mock, mock_repository_save: Mock):
        """
        测试上传接口 - 单文件上传
        ...
        with patch('src.core.security.current_user', spec=CurrentUser):
    ...
    @patch.object(FileRepository, 'save')
    @patch.object(FileStorage, "save")
    def test_multi_upload(self, mock_save: Mock, mock_repository_save: Mock):
        """
        测试上传接口 - 多文件上传
        :param mock_save: 模拟的文件保存接口 - 避免每次测试时新增文件
        :param mock_repository_save: 模拟的数据库保存接口 - 避免测试时向数据库插入数据
    ...
    def test_download(self, mock_send_file: Mock):
        """
        测试文件下载
        ...
        with self.container.file_service.override(mock_file_service), patch('src.core.security.current_user',
                                                                            spec=CurrentUser):
